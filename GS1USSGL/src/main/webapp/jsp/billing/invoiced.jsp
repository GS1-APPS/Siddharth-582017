...
<%
    Collection<? extends Invoice> invoiced = (Collection<? extends Invoice>)request.getAttribute("invoiced");
...
<jsp:include page="/jsp/includes/header.jsp" flush="true">
  <jsp:param name="pageTitle" value="Invoices not yet billed to customer" />
...
<div class="row">
	<div class="col-md-12">
		<h1>Invoices not yet billed to the customer</h1>
		<c:choose>
		<c:when test="<%= invoiced.size() == 0 %>">
			<p>There are currently no invoices that have not yet been billed to the customer.</p>
		...
		<c:otherwise>
			<p class="small">Each of the invoices below has not yet been billed to the customer. After the invoice is billed, 
			   select it below to record that the bill has been sent.</p>
			<table class="table table-striped">
				...
				<tbody>
					<%
					    for (Invoice invoice : invoiced) {
					        String billUri = MvcUriComponentsBuilder.fromMethodName(BillingController.class, "billInvoiceGet", null, null, invoice.getInvoiceId()).toUriString();
						...
						<tr>
							...
							<td><a href="<%= billUri %>" title="Issue a bill to the customer for this invoice"><span class="icon-shopping_cart"></span></a></td>
						</tr>
					<% } %>
				</tbody>
			</table>
